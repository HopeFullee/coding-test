/* 
  ...
  - 첫째와 둘째를 비교, 둘째와 셋째를 비교, 셋째와 넷째를 비교해서 큰 값은 뒤로 작은 값은 앞으로 swap한다.
  
  - 한 번의 단계(parent index)가 수행되면, 가장 큰 원소가 맨 뒤로 이동한다.
  - 따라서, 그 다음 단계에서는 맨 뒤로 이동한 데이터는 정렬에서 제외한다.
  - [핵심] 각 단계를 거칠때마다 가장 큰 값을 하나씩 확실하게 결정하는 것으로 이해할 수 있다.
